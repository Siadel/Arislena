...
- 외교 관련
  ...
  - **호감도**
    ...
    -1000에서 1000 범위인 정수로 표현한다. 원주민만 가지고 있는 스탯이다(따로 호감도 표를 작성해야 한다). -1000에 가까울수록 적대적인 행위를 하고, 1000에 가까울수록 우호적인 행위를 한다. 원주민이 어느 나라에 대해 호감도를 1000

  - **경계도**

    원주민만 갖고 있는 스탯이다. 내정의 경계도와 같다(따로 경계도 표를 작성해야 한다). 정확히는, 원주민은 원래 다른 나라에 대한 호감도와 경계도가 모두 있다. 만약 원주민이 다른 나라의 식민지가 되면, 호감도는 충성도가 되고, 경계도는 그대로 보존된다.
