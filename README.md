 ...
 **개발자는 임의로 규칙을 변경할 수 있다.**

## 게임의 모토와 진행

 공세관(**공**동 **세**계**관**)은 한 행성(보통 그렇다) 내에서 각 플레이어가 만든 설정 아래 가상의 국가를 세우고, 가상의 정부를 세우고, 가상의 인물을 만들어 마치 자신이 역사를 써내려가는 느낌을 주는 게임이다. 이 과정에서 다른 플레이어(국가)와 정치적인 교류를 진행하며 재미를 얻는 것이 가장 큰 모토라고 할 수 있다.

 지난 몇 년간 공세관은 네이버 카페에서 세밀하게 정해지지 않은 규칙을 가지고 잠시 동안 모였다가 흥미가 떨어지면 모두 흩어지는 것을 반복하는 것뿐이었다. 시스템만 문제가 아니었지만, **정해지지 않은 규칙**이라는 것이 지속적인 플레이에 상당히 발목을 잡은 것도 맞았다. 바로 그 문제를 해결하기 위해 프로그램을 짜서 공세관 게임을 진행하려 한 것이 아리슬레나다.

 아리슬레나에는 **시즌**이 있는데, 시작부터 끝까지가 한 시즌이다.

### 시작

 최소한 4명이 모여야 시작할 수 있다. 시작한 날을 0일이라고 한다. 하루가 지날 때마다(지나는 시기는 24시, 23시, 6시 등 어느 때로 해도 된다) `!새날`을 작동시킨다. 봇이 정상적으로 작동한다면, 1일에 원주민이 생긴다.

### 진행

 각 유저는 자신이 원하는 플레이 스타일대로 성장한다.

#### 개인 패배

 유저는 어떤 방식으로든 자신의 나라를 잃으면(데이터상에서 나라가 사라지거나, 나라의 오너가 자신이 아니게 될 때 등) 패배한다.

### 결말

 국제 기술+문화 평균이 120을 넘기거나, 최강국 기술+문화 값이 200을 넘기면 발동한다.

 #### 승리

 위의 조건을 충족한 상태로 `!새날`이 8번 동작하면 시즌을 마칠 수 있는 권리가 유저에게 생긴다. 유저는 어느 때나 시즌을 마칠 수 있게 된다.

#### 취소

 `!새날`이 동작한 시점을 기준으로 위의 조건을 충족하지 못했다면, 카운트를 초기화한다.

#### 패배

 모든 유저가 개인 패배되면 시즌은 패배로 끝난다.
...
1. 게임 운영
   ...
   1. 채널 신설
      새로운 인원이 참가를 희망했을 때, `!건국`을 실행한 후, 맨 밑에 **(닉네임)-(나라명)** 형식으로 카테고리를 만들고, **☆(수도명)** 형식으로 채널을 만든다. 해당 카테고리는 해당 인원에 한해 채널 신설/수정/삭제 권한을 주고, 나머지 모든 인원에 대해 메세지 보기/읽기만 허용하도록 한다.
   2. 채널 삭제
      어느 유저가 패배(나라를 잃음)한다면, 유저의 카테고리를 보존할 것인지 아니할 것인지 의사를 묻고, 보존하지 아니한다고 답하면 카테고리와 이하 모든 채널을 삭제한다.
2. 디코방 운영
...
- 경제 관련
  ...
  - **대출**
    ...
    `!대출` 명령어로 급전을 마련할 수 있다. 수도권 기술/문화 합에 따라 대출받을 수 있는 자금 규모가 늘어난다. 대출에는 대출액을 기준으로 한 단계(~5000, ~15000, ~45000)가 있으며, 각 단계마다 요구하는 수도권 기술/문화 합, 이자율, 최대로 설정할 수 있는 상환 기간이 달라진다.

  - **빚**

    대출을 하면 설정한 대출금과 계산된 이자의 합만큼 빚이 생긴다. 상환량은 빛에 남은 상환 기간을 나눈 값이다.

  - **남은 상환 기간**

    대출을 할 때 설정한 상환 기간만큼 정해진다. `!새날`을 하면 1 감소한다. 상환 기간이 1 이상인 경우, 다시 대출을 할 수 없다. 조기 상환을 하려 할 경우, 일정량의 위약금을 추가로 내야 한다.

- 내정 관련

  - **투자 점수**

    수도권 기술 5, 문화 8당 점수를 1 얻는다. 또, **추출**을 통해서도 1 얻을 수 있다.

  - **투자**

    투자 점수(1)와 자금(2000)을 소비해 일정 이득을 얻는 행동이다.

  - **추출**

    자금(2500)을 소비하고 리스크를 얻는 대신 투자 점수를 얻는 행동이다.

  - **취소**

    자금(800)을 소비하고 투자나 추출한 것을 되돌리는 행동이다.

  - **정책 행동**

    정책 행동은 투자 횟수, 추출 횟수, 취소 횟수를 모두 더한 값이다. 하루에 행할 수 있는 정책 행동은 4다. 4를 넘으면 더 이상 정책에 관련한 행동을 하지 못한다.

  - **충성도**

    지역이 나라에 가지고 있는 충성심을 숫자로 표현한 것이다. -1000에서 1000 범위인 정수로 표현한다. 충성도가 -1000에 가까울수록 반란이 일어날 확률이 높다. 수도권의 충성도는 무조건 1000으로 고정된다.

  - **경계도**

    지역이 나라에 가지고 있는 경계심을 숫자로 표현한 것이다. -1000에서 1000 범위인 정수로 표현한다. 경계도가 -1000이나 1000에 가까울수록 반란이 일어날 확률이 높다. 수도의 경계도는 무조건 0으로 고정된다.

- 외교 관련

  - **호전성**

    Ari_data의 class Formula의 belligerence() 참조

    0 이상인 유리수로 표현한다. 나라 기술과 문화의 상대값, 나라 식량과 나라 기준 식량의 상대값, 나라 자재와 나라 기준 자재의 상대값으로 정해진다. 이 값은 원주민이 유저나 다른 원주민 나라의 호감도를 조정하거나 행동 양식을 정할 때 쓰인다.

    (호전성이 4 이하면 대상이 해이하다고 판단해 대상을 업신여기고(경계도 하락)호전성이 14 이상이면 대상이 전쟁 준비를 한다는 것으로 판단하여, 대상을 경계하는 것으로 계획. 그러나 충분한 데이터가 모이지 않았기 때문에, '이 정도면 전쟁 준비를 할 만하다' 하는 선에서 경계 기준을 정하도록 한다.)

  - **호감도**

    -1000에서 1000 범위인 정수로 표현한다. 나라 스탯으로 판단하며, 원주민만 가지고 있는 스탯이다(따로 호감도 표를 작성해야 한다). -1000에 가까울수록 적대적인 행위를 하고, 1000에 가까울수록 우호적인 행위를 한다. 원주민이 어느 나라에 대해 호감도를 1000만큼 쌓으면, `!새날`이 작동할 때 25% 확률로 해당 나라에 자신 나라에 속한 모든 지역을 식민지화하도록 제안한다.

  - **경계도**

    나라 스탯으로 판단하며, 원주민만 갖고 있는 스탯이다. 내정의 경계도와 같다(따로 경계도 표를 작성해야 한다). 정확히는, 원주민은 원래 다른 나라에 대한 호감도와 경계도가 모두 있다. 만약 원주민이 다른 나라의 식민지가 되면, 호감도는 충성도가 되고, 경계도는 그대로 보존된다.
...
- -200 ~ 200
  ...
  중립적 - 특수한 경우가 아니면 기본값은 이 범위 사이의 값으로 한다.
...
- 관리자 명령어
  ...
  4. 중지 - 관리자만이 명령어를 사용할 수 있게 하는 명령어.
  5. 재개 - 중지한 것을 푸는 명령어.
  6. 건국 - 나라와 수도 지역 데이터를 추가하는 명령어.
  7. **식민**
  8. **편입**
  9. **병합**
  10. **추방**
  11. *새날* - 시즌0의 리프레시. 추가된 기능이 많다.
      1. *자원 회복*
      2. *원주민 생성*
      3. **호감도 조정**
      ...
      6. **지역 자동 매각**
      7. 기타 여럿
  12. **세부열람** - 검색한 나라의 모든 (관리자만 봐야 할 정보까지) 정보를 출력하는 명령어
  13. **경매**
      1. **시작**
      2. **마감**
- 왕 명령어
